...
def particle_potential_velocity(
    u: np.ndarray,
    v: np.ndarray,  # [nx, ny, nm]
    s: np.ndarray,  # [nx, ny, nm]
    alpha_p_cell2d: np.ndarray,  # [nx, ny] - Solid volume fraction
    p,  # Parameter object
    *,
    dt: float | None = None,
    gx: float = 0.0,
    ...
    """
    Simplified Inertial Velocity Calculation:
    1. Assumes Static Gas (Ug = 0, Vg = 0).
    2. Uses a Blend Model (Wen-Yu + Ergun) for drag force calculation.
    3. Integrates gravity and drag to calculate potential velocity after time dt.
    """
    ...
    if dt is None:
        dt = p.dt
    if gy is None:
        gy = -abs(getattr(p, "g", 9.81))

    # Physical constants
    rho_g = p.gas_density  # Gas density
    mu_g = p.gas_viscosity  # Gas viscosity
    rho_p = p.solid_density  # Particle density
    eps = 1e-30  # Epsilon to prevent division by zero

    # Expand dimensions to match [nx, ny, nm] for broadcasting
    # alpha_p_cell2d is originally [nx, ny], needs to be [nx, ny, 1]
    alpha_p3 = alpha_p_cell2d[:, :, np.newaxis]

    # Determine occupancy (where particles exist)
    occ = np.isfinite(s)

    # Calculate fluid volume fraction (alpha_f)
    # alpha_f = 1 - alpha_s
    alpha_f3 = np.clip(1.0 - alpha_p3, 1e-6, 1.0)

    # Safe particle diameter (handle NaNs by replacing with 1.0, though masked later)
    d_safe = np.where(occ, np.maximum(s, eps), 1.0)

    # --- 2. Calculate Relative Velocity & Reynolds Number ---
    Ur = np.sqrt(u * u + v * v)

    # Reynolds Number (Re_p)
    Re = rho_g * Ur * d_safe / (mu_g + eps)
    Re = np.maximum(Re, 1e-12)

    # Standard Drag Coefficient Cd (Schiller-Naumann correlation)
    Cd = 24.0 / Re * (1.0 + 0.15 * Re**0.687)

    # --- 3. Calculate Beta (Drag Coefficient Term) ---

    # A. Wen-Yu Model (Suitable for sparse flow)
    beta_wy = 0.75 * Cd * rho_g * (Ur / (d_safe + eps)) * np.power(alpha_f3, -2.65)

    # B. Ergun Model (Suitable for dense flow / packed bed)
    beta_er = 150.0 * mu_g * (alpha_p3**2) / (alpha_f3 * (d_safe**2) + eps) + 1.75 * rho_g * alpha_p3 * (
        Ur / (d_safe + eps)
    )
    mask_wy = alpha_f3 >= 0.8
    beta = np.where(mask_wy, beta_wy, beta_er)

    # Ensure drag is only applied where particles actually exist
    beta = np.where(occ, beta, 0.0)

    # --- 4. Time Integration ---
    # We solve the equation: dv/dt = g_eff - a*v
    # where 'a' is the drag relaxation coefficient: a = beta / rho_p

    # Calculate relaxation coefficient 'a'
    a = beta / (rho_p + eps)

    # Clip 'a' to prevent numerical instability
    ...
    buoy = (rho_p - rho_g) / (rho_p + eps)
    bx, by = gx * buoy, gy * buoy

    # Analytic Solution for Linear Drag over time step dt:
    # v_new = v_terminal + (v_old - v_terminal) * exp(-a*dt)
    # where v_terminal = force / a
    u_pot = (bx / a) + (u - bx / a) * expf
    v_pot = (by / a) + (v - by / a) * expf

    return np.nan_to_num(u_pot), np.nan_to_num(v_pot)
...
def move_voids(
    ...
    if p.inertia is not False:
        u, v = particle_potential_velocity(u, v, s, nu, p, dt=p.dt, gx=0, gy=-9.81)
    ...
    for axis, d in options:
        ...
        if axis == 1:  # vertical
            ...
            if p.inertia is not False:
                v_new_swapper = np.roll(v, d, axis=axis)
                P = (np.abs(v_new_swapper)) * p.dt / p.dy
                if d == -1:
                    P[v_new_swapper >= 0] = 0  # up
                else:
                    P[v_new_swapper <= 0] = 0  # down
