...
py::tuple move_voids(
    ...
    double P_lr_ref = alpha * P_u_ref;
    double delta_nu = p.attr("delta_limit").cast<double>();
    ...
    for (auto index : indices) {
        ...
        if (!(skip.attr("__getitem__")(py::make_tuple(i, j)).cast<bool>())) {
            py::array_t<int> src = py::array_t<int>({i, j, k});
            if (std::isnan(s_buf(i, j, k))) {
                if (!operators.attr("locally_solid")(s, i, j, p).cast<bool>()) {
                    double P_u = (!std::isnan(s_buf(i, j + 1, k))) ? 
                        P_u_ref * (s_inv_bar[i * ny + j + 1] / s_buf(i, j + 1, k)) : 0;
                    ...
                    double P_l = (!std::isnan(s_buf(l, j, k)) && !stable_left) ?
                    P_lr_ref * (s_buf(l, j, k) / s_bar[l*ny+ j]) : 0;

                    double P_r = (!std::isnan(s_buf(r, j, k)) && !stable_right) ?
                        P_lr_ref * (s_buf(r, j, k) / s_bar[r*ny+ j]) : 0;
